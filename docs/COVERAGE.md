...
| `display: grid` (+ `grid-template-columns`, **`grid-template-rows`**, `gap`/`row-gap`/`column-gap`, `grid-column`, **`grid-row`**, `align-items`/`align-self`) | 🟡 | Column tracks: fixed lengths, `fr`, `auto`, `repeat(N, …)`, **`minmax(min, max)`** (pt/auto floors; pt/fr/auto ceilings; fr floors pinned iteratively). **Row tracks (`grid-template-rows`)**: fixed lengths, `auto`/content, `minmax()` bounds, and **`fr` shares of a definite container height** (`height`/`min-height`; falls back to content when indefinite); implicit rows past the track list stay content-sized. Placement: row-major auto-placement, `span N`, and **line-based `grid-column: A / B`** incl. negative lines (`1 / -1` = full row). **2D placement (`grid-row`)**: line-based row placement and row spans switch the grid to an **occupancy-grid** algorithm — auto items flow around pinned/spanned cells (sparse cursor per CSS §8.5; a row-pinned cell leaves earlier gaps for later auto items), and a row-spanning cell taller than its rows grows the last spanned row (as table `rowspan` does). **Block-axis alignment**: `align-items` + per-item `align-self` (center/end; `stretch`≈start, items not inflated) place items vertically within their cell. No named lines/areas, dense packing, `justify-items`/`justify-self` (items fill their column width), or splitting a row-spanning cell across a page break. |
