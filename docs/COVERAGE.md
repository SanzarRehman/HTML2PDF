...
|---|---|---|
| `body` | ✅ | A real block box: `margin`/`padding`/`background` on `body` apply, and the UA default `margin: 8px` is there when nothing overrides it (its top margin collapses with the first child's, as in a browser). |
| `h1`–`h6`, `p`, `div`, `section`, `article`, `main`, `header`, `footer`, `nav`, `aside`, `address`, `figure`, `figcaption`, `pre` | ✅ | Block flow with margins/padding/borders. UA defaults follow Chrome: base font 16px (12pt); headings `2em`…`0.67em` with `0.67em`…`2.33em` margins; `p`, `blockquote`, `pre`, `ul`, `ol`, `dl`, `figure` get `margin: 1em 0`; generic containers (`div`, `section`, `li`, …) get none. |
| Quirks mode (no `<!DOCTYPE>`) | 🟡 | Detected by the parser and reproduced where it moves layout: a UA-default top margin on `body`'s first in-flow block (and bottom margin on its last) is ignored, as in every browser. Other quirks (table-cell font reset, unitless lengths, line-height quirks) are not modelled. |
...
| `@page` margins, orientation, margin boxes | 🟡 | `size` (`<length>{1,2}`, named sizes `A3`–`A6`/`B4`/`B5`/`JIS-B4`/`JIS-B5`/`letter`/`legal`/`ledger`/`executive`, and the `portrait`/`landscape` keywords), `margin`, plus `@top-left/center/right` and `@bottom-left/center/right` running text. Margin-box `content` supports quoted strings, `counter(page)` / `counter(pages)` with final values, and **`element(<name>)`** — a CSS GCPM **running element**. `position: running(<name>)` lifts an element out of flow and registers it; the referencing margin box lays that subtree out as a real block (nested markup, bold/colour, images, tables, links) once, then stamps it on every page, shrink-to-fit and aligned to its slot's edge, clipped to the page-margin band so it can never paint over the body. A name no margin box references renders nowhere; the last element registered under a name wins. Not yet: named pages, page selectors (`@page :first`), corner boxes, `string-set` / `content: string()`, and auto-sizing the page margin to the running element (it is clipped to the declared margin instead). |
...
| `break-inside` | 🟡 | `avoid` / `avoid-page` on **in-flow flow blocks**: a block that does not fit the space left on the page starts on a fresh page instead of being cut by the page bottom (measured with a dry-run layout, so borders/backgrounds/nested content are exact). A block taller than a whole page still starts a fresh page and then breaks inside it, like Chrome. `auto` (and the column-only `avoid-column`/`avoid-region`) allow breaks and override an `avoid` in the cascade. Not yet: `avoid` on table rows/cells, flex/grid items, and floats (floats never split anyway); no orphans/widows. `break-before`/`break-after` are supported separately (see Forced page breaks below) and are applied first, so a forced break decides the page and `avoid` then checks the fit on it. The legacy `page-break-inside` alias is deliberately not supported. |
| `overflow-wrap`, `word-break` | ✅ | |
| `display` | 🟡 | `none`, `table-*-group`, `flex`, `grid`, and **`inline-block`**. `inline-block`: the element is a block box (padding/border/background/`border-radius`/CSS width·height) that flows on the surrounding line as an atomic item aligned by its content baseline; text flows around it and wraps. First slice covers single-line inline content (badges/buttons/tags/chips/swatches). Not yet: multi-line/wrapping inner content, nested block children, and `vertical-align` other than baseline. |
| `line-height` | 🟡 | Unitless number, `%`, and absolute lengths, on flow blocks and table cells; inherits; extra leading split as half-leading (Chrome-like). `normal` is the face's real ascent+descent box: embedded faces from their metrics; the built-in base-14 Helvetica 1.156 em (the hhea box of the Nimbus Sans a viewer substitutes for it); and the classic macOS faces Helvetica/Times/Courier (zero hhea gap) get Blink's 15% ascent inflation so 12pt Helvetica measures 13.5pt as it does in Chrome. Cells stay `×1.18`. No `normal` *override* of an inherited value, block-level only (no per-inline-run line-height). |
| `font-family` | 🟡 | Per-element: named families resolve to real system faces (embedded + subset, several per document); generics (`serif`/`sans-serif`/`monospace`/…) map to system defaults; `pre`/`code`/`kbd`/`samp` default to monospace. The **whole declared stack** is walked in author order — the first name that resolves to a real face wins, so `Helvetica, Arial, sans-serif` reaches the generic (and its real metrics) on a machine that has neither of the first two. Families a system is unlikely to ship resolve through metric-compatible substitutes, the way fontconfig does (`Helvetica`/`Arial` → Liberation Sans/Nimbus Sans; `Times` → Liberation Serif; `Courier` → Liberation Mono). No per-character walk down the stack — the fallback chain handles missing glyphs. A stack where *nothing* resolves falls back to the document font (base-14), whose `line-height: normal` is a flat 1.35 em. |
...
| `display: flex` (+ `flex`, `flex-grow`, `flex-shrink`, `flex-basis`, `order`, `flex-wrap`, `flex-flow`, `justify-content`, `align-items`, `align-self`, `align-content`, `gap`, `flex-direction`) | 🟡 | Row: grow/basis sizing, **`flex-shrink`** (overflow shrink weighted by `flex-shrink × base`; `flex-shrink: 0` pins an item), justify-content, **align-items** + per-item **`align-self`** (center/end via measure pass), **`order`** (stable re-sequencing before wrap/placement, grid items too), **`flex-wrap: wrap`** / **`wrap-reverse`** (greedy lines by base size; reverse stacks lines bottom-to-top; justify/align apply per line; `gap` doubles as cross-axis line gap), **`align-content`** (cross-axis line distribution when a definite container height leaves free space), inline (`span`) children promoted to items, anonymous text items. **Column**: vertical stack with `gap` (no height grow/justify/`align-content`/`wrap-reverse`). A block-level `<img>` child is an item in its own right (a replaced element: no grow, no stretch, base size = its resolved width). Limits: `width`/`height` don't seed `flex-basis` (only explicit `flex-basis`); a `<table>` inside a flex row is still skipped; `wrap-reverse` reverses line order but not the per-line align sense; no cross-page rows. |
| `display: grid` (+ `grid-template-columns`, **`grid-template-rows`**, **`grid-template-areas`**, `gap`/`row-gap`/`column-gap`, `grid-column`, **`grid-row`**, **`grid-area`**, `align-items`/`align-self`) | 🟡 | Column tracks: fixed lengths, `fr`, `auto`, `repeat(N, …)`, **`minmax(min, max)`** (pt/auto floors; pt/fr/auto ceilings; fr floors pinned iteratively). **Row tracks (`grid-template-rows`)**: fixed lengths, `auto`/content, `minmax()` bounds, and **`fr` shares of a definite container height** (`height`/`min-height`; falls back to content when indefinite); implicit rows past the track list stay content-sized. Placement: row-major auto-placement, `span N`, and **line-based `grid-column: A / B`** incl. negative lines (`1 / -1` = full row). **2D placement (`grid-row`)**: line-based row placement and row spans switch the grid to an **occupancy-grid** algorithm — auto items flow around pinned/spanned cells (sparse cursor per CSS §8.5; a row-pinned cell leaves earlier gaps for later auto items), and a row-spanning cell taller than its rows grows the last spanned row (as table `rowspan` does). **Named areas (`grid-template-areas`)**: each name resolves to its bounding rectangle; `grid-area: <name>` pins an item there regardless of DOM order (the classic header/sidebar/main/footer layout); the numeric `grid-area: r / c / r2 / c2` form and `.` empty cells are supported, and areas imply extra `auto` columns when `grid-template-columns` is shorter. **Block-axis alignment**: `align-items` + per-item `align-self` (center/end; `stretch`≈start, items not inflated) place items vertically within their cell. No named grid *lines* (only areas), dense packing, `justify-items`/`justify-self` (items fill their column width), or splitting a row-spanning cell across a page break. |
...
| `columns` (multi-col) | ❌ | (`flex-shrink` and `order` shipped — see the `display: flex` row.) |
...
| Forced page breaks (`break-before`/`break-after`, `page-break-before`/`page-break-after`) | 🟡 | `always`/`page` start a new page before/after the block, in flow and at any nesting depth; `left`/`right`/`recto`/`verso` degrade to a plain break (no left/right page distinction). Back-to-back breaks collapse and a trailing `break-after` leaves no blank page. Not on out-of-flow (absolute/fixed) or floated boxes, and not between table rows. Composes with `break-inside: avoid` (above): `break-before` is applied first, then the keep-whole fit check runs on the page it chose. |
...
| Running headers/footers + page numbers | 🟡 | Standards-style `@page` margin boxes with final `counter(page)` / `counter(pages)` values, and **CSS GCPM running elements** (`position: running()` + `content: element()`) for headers that are markup — a logo, a stamp, a two-column band. Static `position: fixed` headers/footers also repeat on every page. No `string-set` running chapter titles, no `@page :first`, no visible generated TOC yet. |
