...
7. Box generation from computed `display`: flow content (non-table documents) is
   ...
   text alignment). **Done**, then extended by the nested box tree below.
7b. Nested flow box tree (`box_tree.rs`): flow content now lowers to a nested
   tree of block boxes whose leaves are styled inline runs, laid out recursively
   (nesting, list/blockquote indentation, list markers, per-run color/size,
   whitespace-collapsing wrap). See **ADR 0004**. The table path keeps its
   specialized layout. **Done.**
