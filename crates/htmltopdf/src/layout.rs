...
fn layout_box_children(
    ...
    for child in children {
        // Fragmentation. Out-of-flow boxes (absolute/fixed) and floats are not
        // in the block progression, so neither a forced break nor a keep-whole
        // request declared on one has anything to act on.
        let fragmentable = match child {
            BoxChild::Block(block) if block.float_dir.is_none() && !is_out_of_flow(block) => {
                Some(block)
            }
            _ => None,
        };
        let forced = fragmentable
            .map(|block| (block.break_before, block.break_after))
            .unwrap_or((None, None));

        // `break-before` first: it decides which page the block starts on.
        if forced.0.is_some() {
            force_page_break(pages, y, carried, options);
        }
        // Then `break-inside: avoid` on that page — keep the block whole by
        // starting it on a fresh page when the page bottom would otherwise split
        // it. Order matters: a forced break may already have supplied the room.
        if let Some(block) = fragmentable {
            if block.break_inside_avoid {
